...
class Data:
    ...
    def create_database(self):
        ...
        try:
            self.cursor.execute(
                f'INSERT INTO {self.tbl_name}'
                '(file_path, file_name, input_i, input_tp, input_lra, input_thresh, target_offset, waveform_path,'
                'audio_streams, black_start, black_end, black_duration, silence_start, silence_end, silence_duration,'
                'freeze_start, freeze_end, freeze_duration, marks)'
                'VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
                ('file_path', 'file_name', 'input_i', 'input_tp', 'input_lra', 'input_thresh',
                 'target_offset', 'waveform_path', 'audio_streams', 'black_start', 'black_end',
                 'black_duration', 'silence_start', 'silence_end', 'silence_duration',
                 'freeze_start', 'freeze_end', 'freeze_duration', 'marks'))

            # self.cursor.execute('INSERT INTO Video DEFAULT VALUES')
        except Exception:
            pass
    ...
    def add_video(self, file_path):
        ...
        try:
            self.cursor.execute(f'INSERT INTO {self.tbl_name} (file_path, file_name) VALUES (?,?)', (file_path, file_name,))
            # self.cursor.execute(
            #     'UPDATE Video SET input_i = ?, input_tp = ?, input_lra = ?, input_thresh = ?, target_offset = ?, waveform_path = ? WHERE file_path = ?',
            #     ('нет данных', 'нет данных', 'нет данных', 'нет данных', 'нет данных', 'нет данных', file_path))
        except Exception:
    ...
    def add_ffprobe_data(self, header, data, file_path):
        try:
            self.cursor.execute(f'''
            ALTER TABLE {self.tbl_name} ADD COLUMN "{header}" TEXT
            DEFAULT 'нет данных';
            ''')
        except Exception:
            pass
        ...
        try:
            self.cursor.execute(f'UPDATE {self.tbl_name} SET {header} = ? WHERE file_path = ?', (header, 'file_path'))
        except Exception:
            print('Ошибка добавления ключей')
            pass

        try:
            self.cursor.execute(f'UPDATE {self.tbl_name} SET {header} = ? WHERE file_path = ?', (data, file_path))
        except Exception:
    ...
    def add_data(self, header, data, file_path):
        try:
            self.cursor.execute(f'UPDATE {self.tbl_name} SET {header} = ? WHERE file_path = ?', (str(data), file_path))
        except Exception:
    ...
    def read_bd(self, header, file_path):
        try:
            self.cursor.execute(f'SELECT {header} FROM {self.tbl_name} WHERE file_path = ?', (file_path,))
    ...
    def read_all_data(self, file_path):
        self.cursor.execute(f'SELECT * FROM {self.tbl_name} WHERE file_path = ?', (file_path,))
